...
def compute_A(item_profits, item_weights, capacity, set_size):
    ...
    Uses a 0/1 knapsack DP. numpy evaluates the full RHS into a temporary
    before assignment, so the in-place slice update is safe and equivalent
    to the reverse-order scalar loop.
    """
...
def solve_kpdfs(instance_path):
    """
    Solve the KPDFS via the DP described in the algorithm overview.
    ...
    for t, fs in enumerate(all_sets):
        ...
        # Note: -inf - finite = -inf, so undefined entries stay -inf.
        ...
        for W, s in np.argwhere(np.isfinite(val)):
            ...
            # for all b' >= W and k' >= v simultaneously.
            f_curr[W:, v:] = np.maximum(
                f_curr[W:, v:],
                f_prev[:b + 1 - W, :k + 1 - v] + val[W, s]
            )

        f_prev = f_curr
        elapsed = time.time() - t0
        n_valid = int(np.isfinite(f_curr).sum())
        print(
            f"  Set {t + 1:>{len(str(l))}}/{l}  "
            f"({n_items:>4} items, h={h}, d={d})  "
            f"reachable states: {n_valid}  "
...
def main():
    parser = argparse.ArgumentParser(
        description='Solve a KPDFS instance via dynamic programming.'
    )
    parser.add_argument('instance', help='Path to instance file')
    args = parser.parse_args()

    result = solve_kpdfs(args.instance)
